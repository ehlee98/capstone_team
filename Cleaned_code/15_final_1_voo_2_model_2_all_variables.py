...
data = pd.read_csv(f"data/{ticker}_prepared_data.csv")
data = data.fillna(0).drop(columns=['level_0', 'index'], errors='ignore')
data_full = data.drop(columns=['Date', 'Log_Return','Next_day_Adj_Open_price'], errors='ignore')
...
feature_names = data_full.columns[1:]  # Exclude 'Adj_Closed_price'
importance_scores = {}
...
output_filename = f"results/{ticker}_importance_scores.csv"
...
data_selected=data[select_top_features]
...
trading_strategy(ticker,variable_method,model_name,df,long_threshold= quantiles[0.75],short_threshold=quantiles[0.1],strategy='long_only')
...
result = adfuller(data_selected['Adj_Closed_price'])
...
dataset_diff = data_selected['Adj_Closed_price'].diff().dropna()
...
# Fit ARIMAX model
model = ARIMA(data_selected['Adj_Closed_price'],exog=data_selected.iloc[:, 1:], order=(1, 1, 1))  # Adjust order (p, d, q) as needed
model_fit = model.fit()
# Save the model
...
results_df = pd.DataFrame({
    ...
    "Actual": data_selected['Adj_Closed_price'].iloc[1:].reset_index(drop=True),
